...
from gi.repository import Gst, GObject, GstBase, GstAudio, GstVideo
from numpy_ringbuffer import RingBuffer
from matplotlib import pyplot as plt
from matplotlib.backends.backend_agg import FigureCanvasAgg

# Dibujar el audio usando matplotlib construyendo un plugin de gstreamer especial para ello

# python3 -m pip install matplotlib numpy_ringbuffer
# Fuente del ejemplo: https://mathieuduponchelle.github.io/2018-02-15-Python-Elements-2.html?gi-language=undefined

# Otra librería interesante es: http://vispy.org/
...
class AudioPlotFilter(GstBase.BaseTransform):
    ...
    __gsttemplates__ = (Gst.PadTemplate.new("src", Gst.PadDirection.SRC, Gst.PadPresence.ALWAYS, OCAPS), Gst.PadTemplate.new("sink", Gst.PadDirection.SINK, Gst.PadPresence.ALWAYS, ICAPS))
    __gproperties__ = {"window-duration": (float, "Window Duration", "Duration of the sliding window, in seconds", 0.01, 100.0, DEFAULT_WINDOW_DURATION, GObject.ParamFlags.READWRITE)
    }
    ...
    def do_transform(self, inbuf, outbuf):
        if not self.h:
            self.h, = self.ax.plot(np.array(self.ringbuffer), lw=0.5, color='k', path_effects=[pe.Stroke(linewidth=1.0, foreground='g'), pe.Normal()])
        else:
    ...
    def do_set_caps(self, icaps, ocaps):
        ...
        self.converter = GstAudio.AudioConverter.new(GstAudio.AudioConverterFlags.NONE, in_info, self.convert_info, None)
        ...
        self.samplesperbuffer = Gst.util_uint64_scale_int(in_info.rate * in_info.channels, out_info.fps_d, out_info.fps_n)
        self.next_offset = self.samplesperbuffer
        self.cur_offset = 0
        self.buf_offset = 0
...
class PipeLineTest(Gst.Pipeline):
    def __init__(self):
        ...
        self.add(autovideosink)

        autoaudiosrc.link(audioplot)
        audioplot.link(videoconvert)
        videoconvert.link(autovideosink)

        bus = self.get_bus()
        bus.add_signal_watch()
        bus.connect("message", self.__sync_message)

        self.set_state(Gst.State.PLAYING)

        controller = GLib.timeout_add(50, self.__handle)
